...
async function callLLM(message) {
  ...
  const systemMsg = { role: 'system', content: 'Tu es un assistant commercial pour l application IPCE. Tu aides l administrateur avec ses données business (CA, offres, BC, RDV, collectes). Tu réponds en français, tu es précis et utile. Tu n es pas un modèle de sécurité — tu es un assistant métier.' };
...
// --- App Guide Sections (RAG-lite pour websearch_agent) ---
...
// Plus spécifique en dernier → en cas d'égalité de score, la section la plus spécifique gagne.
