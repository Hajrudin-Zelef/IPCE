...
async function chatStream(message, history, db, mode = 'free', godmode = false, forceModel = null, res) {
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    if (!model) {
      safeWrite(`data: ${JSON.stringify({ error: 'Aucun modele disponible. Configurez OPENROUTER_API_KEY, GROQ_API_KEY ou NVIDIA_API_KEY dans .env', done: true })}\n\n`);
      safeEnd();
      return;
    }
    ...
    try {
      ...
      safeWrite(`data: ${JSON.stringify({ done: true, model: model.id, modelName: model.name, provider: model.provider })}\n\n`);
      safeEnd();
      return;
    } catch (err) {
      ...
      if (i === maxAttempts - 1) {
        safeWrite(`data: ${JSON.stringify({ error: err.message || 'Erreur IA', done: true })}\n\n`);
        safeEnd();
      }
    }
  }
